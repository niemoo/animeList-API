...
export default function AnimeListCard({ api }) {
  return (
    <div className="grid md:grid-cols-5 sm:grid-cols-3 grid-cols-2 gap-5 w-full">
      {api.data?.map((data) => {
        return (
          <Link href={`/${data.mal_id}`} className="cursor-pointer transition-all" key={data.mal_id}>
            <Image src={data.images.webp.image_url} alt="..." width={250} height={350} className="w-full max-h-64 object-cover rounded rounded-md" />
